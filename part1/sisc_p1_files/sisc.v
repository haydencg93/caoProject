...
module sisc (clk, rst_f, ir);
  ...
  ctrl ctrl_unit (.clk(clk), .rst_f(rst_f), .opcode(ir[31:28]), .mm(ir[27:24]), 
                  .stat(stat_out), .rf_we(rf_we), .alu_op(alu_op), .wb_sel(wb_sel));

  rf rf_unit (.clk(clk), .read_rega(ir[19:16]), .read_regb(ir[15:12]), 
              .write_reg(ir[23:20]), .write_data(write_data), .rf_we(rf_we), 
              .rsa(rsa), .rsb(rsb));

  alu alu_unit (.clk(clk), .rsa(rsa), .rsb(rsb), .imm(ir[15:0]), .alu_op(alu_op), 
